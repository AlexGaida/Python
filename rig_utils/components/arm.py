...
def get_joint_positions(joint_1, joint_2, joint_3):
    """
    get joints positions from the three arguments provided
    :param joint_1:
    :param joint_2:
    :param joint_3:
    :return: <tuple> joint positions
    """
    positions = ()
...
def build(joint_1, joint_2, joint_3, name="", prefix_name="", suffix_name=""):
    ...
    for fk_joint in fk_joints:
        fk_ctrl = control_shape_utils.create_control_at_transform(fk_joint, name=fk_name)
        constraint_utils.parent_constraint(fk_ctrl['controller'], fk_joint)
    ik_name = name + '_ik'
    # return controller and group_names keys
    ik_ctrl = control_shape_utils.create_control_at_transform(ik_joints[-1], name=ik_name, shape_name='sphere')
